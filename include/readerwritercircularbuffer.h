...
namespace moodycamel
{
	...
	public:
		...
		// Blocks the current thread until there's one or more things to dequeue, then dequeues the latest (and throws the rest).
		// returns the number of times items have been dequeued.
		// Warning: items must be default-destructible, or memory will be leaked.
		// Thread-safe when called by consumer thread.
		// No exception guarantee (state will be corrupted) if assignment operator of U throws.
		template <typename U>
		int wait_dequeue_latest(U &item)
		{
			bool success = false;
			int counter = 0;
			while ((size_approx() > 0))
