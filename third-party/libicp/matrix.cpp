...
static FLOATT sqrarg;
#define SQR(a) ((sqrarg = (a)) == 0.0 ? 0.0 : sqrarg * sqrarg)
static FLOATT maxarg1, maxarg2;
...
Matrix::Matrix(const int32_t m_, const int32_t n_, const FLOATT *val_)
{
  allocateMemory(m_, n_);
  int32_t k = 0;
...
Matrix::Matrix(const Matrix &M)
{
  ...
  for (int32_t i = 0; i < M.m; i++)
    memcpy(val[i], M.val[i], M.n * sizeof(FLOATT));
}
...
void Matrix::getData(FLOATT *val_, int32_t i1, int32_t j1, int32_t i2, int32_t j2)
{
  if (i2 == -1)
    i2 = m - 1;
  if (j2 == -1)
...
void Matrix::setVal(FLOATT s, int32_t i1, int32_t j1, int32_t i2, int32_t j2)
{
  if (i2 == -1)
    i2 = m - 1;
  if (j2 == -1)
...
void Matrix::setDiag(FLOATT s, int32_t i1, int32_t i2)
{
  if (i2 == -1)
    i2 = min(m - 1, n - 1);
  for (int32_t i = i1; i <= i2; i++)
...
Matrix Matrix::rotMatX(const FLOATT &angle)
{
  FLOATT s = sin(angle);
  FLOATT c = cos(angle);
  Matrix R(3, 3);
...
Matrix Matrix::rotMatY(const FLOATT &angle)
{
  FLOATT s = sin(angle);
  FLOATT c = cos(angle);
  Matrix R(3, 3);
...
Matrix Matrix::rotMatZ(const FLOATT &angle)
{
  FLOATT s = sin(angle);
  FLOATT c = cos(angle);
  Matrix R(3, 3);
...
Matrix Matrix::operator*(const FLOATT &s)
{
  Matrix C(m, n);
  for (int32_t i = 0; i < m; i++)
    for (int32_t j = 0; j < n; j++)
...
bool Matrix::solve(const Matrix &M, FLOATT eps)
{
...
bool Matrix::lu(int32_t *idx, FLOATT &d, FLOATT eps)
{
  ...
  int32_t i, imax, j, k;
  FLOATT big, dum, sum, temp;
  FLOATT *vv = (FLOATT *)malloc(n * sizeof(FLOATT)); // vv stores the implicit scaling of each row.
  d = 1.0;
  for (i = 0; i < n; i++)
...
void Matrix::svd(Matrix &U2, Matrix &W, Matrix &V)
{
  ...
  FLOATT *w = (FLOATT *)malloc(n * sizeof(FLOATT));
  FLOATT *rv1 = (FLOATT *)malloc(n * sizeof(FLOATT));

  int32_t flag, i, its, j, jj, k, l, nm;
  FLOATT anorm, c, f, g, h, s, scale, x, y, z;
  ...
  { // Diagonalization of the bidiagonal form: Loop over singular values,
    ...
    { // and over allowed iterations.
      ...
      { // Test for splitting.
        nm = l - 1;
        if ((FLOATT)(fabs(rv1[l]) + anorm) == anorm)
        {
          flag = 0;
          break;
        }
        if ((FLOATT)(fabs(w[nm]) + anorm) == anorm)
        {
          break;
        }
      }
  ...
  int32_t s2, inc = 1;
  FLOATT sw;
  FLOATT *su = (FLOATT *)malloc(m * sizeof(FLOATT));
  FLOATT *sv = (FLOATT *)malloc(n * sizeof(FLOATT));
  do
  {
    inc *= 3;
    inc++;
...
void Matrix::allocateMemory(const int32_t m_, const int32_t n_)
{
  ...
  val = (FLOATT **)malloc(m * sizeof(FLOATT *));
  val[0] = (FLOATT *)calloc(m * n, sizeof(FLOATT));
...
FLOATT Matrix::pythag(FLOATT a, FLOATT b)
{
  FLOATT absa, absb;
  absa = fabs(a);
  absb = fabs(b);
