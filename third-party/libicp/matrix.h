...
// typedef double FLOATT; // double precision
typedef float FLOATT; // single precision
...
public:
  ...
  Matrix(const int32_t m, const int32_t n, const FLOATT *val_); // init mxn matrix with values from array 'val'
  ...
  void getData(FLOATT *val_, int32_t i1 = 0, int32_t j1 = 0, int32_t i2 = -1, int32_t j2 = -1);
  ...
  void setVal(FLOATT s, int32_t i1 = 0, int32_t j1 = 0, int32_t i2 = -1, int32_t j2 = -1);

  // set (part of) diagonal to scalar, -1 as end replaces whole diagonal
  void setDiag(FLOATT s, int32_t i1 = 0, int32_t i2 = -1);
  ...
  static Matrix rotMatX(const FLOATT &angle);
  static Matrix rotMatY(const FLOATT &angle);
  static Matrix rotMatZ(const FLOATT &angle);
  ...
  Matrix operator*(const FLOATT &s); // multiply with scalar
  Matrix operator/(const Matrix &M); // divide elementwise by matrix (or vector)
  Matrix operator/(const FLOATT &s); // divide by scalar
  Matrix operator-();                // negative matrix
  Matrix operator~();                // transpose
  FLOATT l2norm();                   // euclidean norm (vectors) / frobenius norm (matrices)
  FLOATT mean();                     // mean of all elements in matrix
  ...
  FLOATT det();                                          // returns determinant of matrix
  bool solve(const Matrix &M, FLOATT eps = 1e-20);       // solve linear system M*x=B, replaces *this and M
  bool lu(int32_t *idx, FLOATT &d, FLOATT eps = 1e-20);  // replace *this by lower upper decomposition
...
private:
  ...
  void releaseMemory();
  inline FLOATT pythag(FLOATT a, FLOATT b);
};
