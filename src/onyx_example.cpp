...
int wmain(int argc, ORTCHAR_T* argv[]) {
  ...
  try {
    ...
    t1.stop();
    std::cout << "Time: " << t1.getElapsedTimeInMilliSec() << std::endl;
    // std::cout << output_tensors.size() << std::endl;
    // double-check the dimensions of the output tensors
    // NOTE: the number of output tensors is equal to the number of output nodes specifed in the Run() call
    assert(output_tensors.size() == output_names.size() && output_tensors[0].IsTensor());
    // Get the inference result
    float* floatarr1 = output_tensors[0].GetTensorMutableData<float>();
    std::vector<float> result1(floatarr1, floatarr1 + 63);
    float* floatarr2 = output_tensors[0].GetTensorMutableData<float>();
    std::vector<float> result2(floatarr2, floatarr2 + 1);
    float* floatarr3 = output_tensors[0].GetTensorMutableData<float>();
    std::vector<float> result3(floatarr3, floatarr3 + 1);
    float* floatarr4 = output_tensors[0].GetTensorMutableData<float>();
    std::vector<float> result4(floatarr4, floatarr4 + 63);
