...
def mls_affine_deformation(vy, vx, p, q, alpha=1.0, eps=1e-8):
    ...
    w = (
        1.0
        / (np.sum((reshaped_p - reshaped_v).astype(np.float32) ** 2, axis=1) + eps)
        ** alpha
    )  # [ctrls, grow, gcol]
    ...
    out_A = mul_right.reshape(2, ctrls, grow, gcol, 1, 1)[
        0
    ]  # [ctrls, grow, gcol, 1, 1]
    A = np.matmul(
        np.matmul(reshaped_mul_left, inv_pTwp), reshaped_mul_right, out=out_A
    )  # [ctrls, grow, gcol, 1, 1]
...
def mls_similarity_deformation(vy, vx, p, q, alpha=1.0, eps=1e-8):
    ...
    mul_right = np.concatenate(
        (reshaped_vpstar, reshaped_neg_vpstar_verti), axis=1
    )  # [2, 2, grow, gcol]
    ...
    for i in range(ctrls):
        ...
        mul_left = np.concatenate(
            (reshaped_phat[i], reshaped_neg_phat_verti), axis=0
        )  # [2, 2, grow, gcol]

        A = np.matmul(
            (reshaped_w[i] * mul_left).transpose(2, 3, 0, 1),
            mul_right.transpose(2, 3, 0, 1),
        )  # [grow, gcol, 2, 2]

        qhat = reshaped_q[i] - qstar  # [2, grow, gcol]
        reshaped_qhat = qhat.reshape(1, 2, grow, gcol).transpose(
            2, 3, 0, 1
        )  # [grow, gcol, 1, 2]
...
def mls_rigid_deformation(vy, vx, p, q, alpha=1.0, eps=1e-8):
    ...
    for i in range(ctrls):
        ...
        mul_left = np.concatenate(
            (reshaped_phat, reshaped_neg_phat_verti), axis=0
        )  # [2, 2, grow, gcol]

        A = np.matmul(
            (reshaped_w * mul_left).transpose(2, 3, 0, 1),
    ...
    transformers[0][nan_mask] = np.interp(
        nan_mask_flat, nan_mask_anti_flat, transformers[0][~nan_mask]
    )
    transformers[1][nan_mask] = np.interp(
        nan_mask_flat, nan_mask_anti_flat, transformers[1][~nan_mask]
    )
